...
- vault 文件列表通过内置的 TypeScript Synology File Station client 获取（polling 模式）
- 本地不直接扫描 vault 目录，而是调用 DSM WebAPI 获取文件列表
- 对比 `vault_files` 表中的记录，识别新增/删除/变更
- 需要下载到本地处理的文件会先落到 `VAULT_PATH` 指向的本地 cache，再交给后续解析与 workflow
